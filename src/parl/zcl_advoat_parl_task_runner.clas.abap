...
CLASS zcl_advoat_parl_task_runner DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: zif_advoat_parl_task_runner.
    ...
    CLASS-METHODS:
      ...
      new
        IMPORTING
          server_group     TYPE zif_advoat_ty_global=>ty_server_group OPTIONAL
          task_prefix      TYPE string OPTIONAL
          max_tasks        TYPE i DEFAULT 10
          handler_class    TYPE string
          handler_method   TYPE seocpdname
          results_receiver TYPE REF TO zif_advoat_parl_result_recv OPTIONAL
        RETURNING
          VALUE(result)    TYPE REF TO zif_advoat_parl_task_runner
        RAISING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      server_group    TYPE zif_advoat_ty_global=>ty_server_group,
      task_number     TYPE i,
      task_prefix     TYPE string,
      free_tasks      TYPE i,
      max_tasks       TYPE i,
      initialized     TYPE abap_bool,
      result_receiver TYPE REF TO zif_advoat_parl_result_recv,
      handler_def     TYPE zif_advoat_ty_global=>ty_parallel_handler.
    ...
    METHODS:
      constructor
        IMPORTING
          ...
          handler_class    TYPE string
          handler_method   TYPE seocpdname
          results_receiver TYPE REF TO zif_advoat_parl_result_recv
        RAISING
          zcx_advoat_exception,
...
CLASS zcl_advoat_parl_task_runner IMPLEMENTATION.

  METHOD new.
    DATA(task_runner) = NEW zcl_advoat_parl_task_runner(
  ...
  METHOD zif_advoat_parl_task_runner~get_max_tasks.
    result = max_tasks.
  ENDMETHOD.


  METHOD zif_advoat_parl_task_runner~set_result_receiver.
    me->result_receiver = receiver.
  ENDMETHOD.


  METHOD zif_advoat_parl_task_runner~run.
    ...
    DO.
      CALL FUNCTION 'ZADVOAT_NEW_TASK'
  ...
  METHOD zif_advoat_parl_task_runner~wait_until_finished.
  ...
  METHOD constructor.
    ...
    me->task_prefix = COND #(
      WHEN task_prefix IS INITIAL THEN |ADVOAT_{ sy-datum }| ELSE task_prefix ).
    me->server_group = server_group.

    me->handler_def = zcl_advoat_parl_proc_utils=>get_parallel_handler(
      handler_class  = handler_class
      handler_method = handler_method ).
  ...
  METHOD on_end_of_task.
    ...
    RECEIVE RESULTS FROM FUNCTION 'ZADVOAT_NEW_TASK'
    IMPORTING
